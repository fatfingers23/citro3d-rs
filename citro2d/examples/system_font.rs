...
fn main() {
    ...
    while apt.main_loop() {
        ...
        if x <= 0.0 || x + c2d_text.width > SCREEN_WIDTH as f32 {
            dx = -dx;
        }
        ...
        citro2d_instance.render_target(&mut top_target, |_instance, render_target| {
            ...
            unsafe {
                citro2d_sys::C2D_TextBufClear(c2d_text_buffer);
                citro2d_sys::C2D_DrawText(&c2d_text as *const _, 0, x, y, 0.5, 1.0, 1.0);
            }
        });
